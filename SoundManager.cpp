...
SoundManager::SoundManager()
{
	...
	//When the player is 50 px away sound is full vol
	float minDistance = 150;

	//The sound reduces steadily
	float attenuation = 15;

	//Set all the attenuation
...
void SoundManager::playFire(Vector2f emitterLocation, Vector2f listernerLocation)
{
	//Where is listener thomas
	Listener::setPosition(listernerLocation.x, listernerLocation.y, 0.0f);

	switch (m_NextSound)
	{
	case 1:
		//Locate/move the sourec of sound
	...
	//Increament the next fire sound
	m_NextSound++;
	if (m_NextSound > 3)
	{
		m_NextSound = 1;
